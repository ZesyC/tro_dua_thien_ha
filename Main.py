...
from ConsistencyModel import ConsistencyDenoise, ConsistencyDiffusion, EMAModel
...
class Coach:
	...
	def prepareModel(self):
		...
		if args.use_consistency:
			# Tính sigma_data từ dữ liệu thực tế
			sigma_data = self.handler.diffusionData.data.std().item()
			log('Consistency Model: sigma_data = %.6f' % sigma_data)

			# Tổng số bước huấn luyện CM cho curriculum schedule
			diff_steps_per_epoch = len(self.handler.diffusionLoader.dataset) // args.batch
			total_cm_steps = args.epoch * diff_steps_per_epoch

			self.diffusion_model = ConsistencyDiffusion(
			...
			self.denoise_model_image = ConsistencyDenoise(
				in_dims, out_dims, args.d_emb_size,
				sigma_data=sigma_data, sigma_min=args.cm_sigma_min,
				norm=args.norm
			).cuda()
			self.ema_model_image = EMAModel(self.denoise_model_image, decay=args.ema_decay)
			self.denoise_opt_image = torch.optim.Adam(self.denoise_model_image.parameters(), lr=args.lr, weight_decay=0)

			self.denoise_model_text = ConsistencyDenoise(
				in_dims, out_dims, args.d_emb_size,
				sigma_data=sigma_data, sigma_min=args.cm_sigma_min,
				norm=args.norm
			).cuda()
			self.ema_model_text = EMAModel(self.denoise_model_text, decay=args.ema_decay)
			self.denoise_opt_text = torch.optim.Adam(self.denoise_model_text.parameters(), lr=args.lr, weight_decay=0)

			if args.data == 'tiktok':
				self.denoise_model_audio = ConsistencyDenoise(
					in_dims, out_dims, args.d_emb_size,
					sigma_data=sigma_data, sigma_min=args.cm_sigma_min,
					norm=args.norm
				).cuda()
				self.ema_model_audio = EMAModel(self.denoise_model_audio, decay=args.ema_decay)
	...
	def trainEpoch(self):
		...
		for i, batch in enumerate(diffusionLoader):
			...
			if args.use_consistency:
				diff_loss_image, gc_loss_image = self.diffusion_model.training_losses(self.denoise_model_image, self.ema_model_image, batch_item, iEmbeds, batch_index, image_feats)
				diff_loss_text, gc_loss_text = self.diffusion_model.training_losses(self.denoise_model_text, self.ema_model_text, batch_item, iEmbeds, batch_index, text_feats)
				if args.data == 'tiktok':
					diff_loss_audio, gc_loss_audio = self.diffusion_model.training_losses(self.denoise_model_audio, self.ema_model_audio, batch_item, iEmbeds, batch_index, audio_feats)
			else:
			...
			# Cập nhật EMA models sau mỗi bước gradient
			if args.use_consistency:
				self.ema_model_image.update(self.denoise_model_image)
				self.ema_model_text.update(self.denoise_model_text)
				if args.data == 'tiktok':
					self.ema_model_audio.update(self.denoise_model_audio)
